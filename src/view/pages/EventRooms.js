...
const EventRooms = ({eventRoom,setChatRoom,isLoaded,setLoading,setPageTitle}) => {
  ...
  return (
    ...
    {(filterAvail===false)?<button class='font-semibold h-fit w-42 rounded-md p-1 bg-white ml-8 mt-8 self-center justify-self-center md:mt-0' onClick={()=>{setFilterAvail(true)}}>❌ With Availability</button>:
    <button class='font-semibold h-fit w-42 rounded-md p-1 bg-orange-400 ml-8 mt-8 self-center justify-self-center md:mt-0 shadow-inner border-2 border-orange-600'  onClick={()=>{setFilterAvail(false)}}>✔️ With Availability</button>}
